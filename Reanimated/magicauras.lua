...
loadstring(game:HttpGet("https://firox.cf/Reanimations/NormalReanimate.lua"))()
		...
		local Hats = {
			gun2 = Character:WaitForChild("Evil Aura"),

		}
		Hats.gun2.Handle.AccessoryWeld:Remove()
		local function align(i,v)
			...
			AO.MaxTorque = 1
			AO.MaxAngularVelocity = 9999999
			AO.Responsiveness = 100
		end
		align(Hats.gun2.Handle, Character["Left Arm"])

		Hats.gun2.Handle.Attachment.Position = Vector3.new(30,0,0)
		...
		local Hats = {
			gun = Character:WaitForChild("Northern Star"),
		...
		Hats.gun.Handle.Attachment.Position = Vector3.new(30,0,0)
		...
		local Anims = {
			['Idle'] = PreloadAnimation(5866860127);
		}
		...
		Connections['Run'] = Humanoid['Running']:Connect(function(Speed)
			if Speed > 100 and Dancing ~= true and Anims['Walk'].Stopped ~= false and runnning ~= true then
				Anims['Idle']:Stop()
				Anims['Jump']:Stop()
				Anims['Fall']:Stop()
				Anims['Run']:Stop()
				Anims['Walk']:Play()
			elseif Speed < 100 and Dancing ~= true and Anims['Walk'].Stopped ~= true and runnning ~= true then
				Anims['Walk']:Stop()
				Anims['Jump']:Stop()
				Anims['Fall']:Stop()
				Anims['Run']:Stop()
				Anims['Idle']:Play()
			elseif Speed < 100 and Dancing ~= true and Anims['Jump'].Stopped ~= true or Anims['Fall'].Stopped ~= true then
				Anims['Walk']:Stop()
				Anims['Jump']:Stop()
				Anims['Fall']:Stop()
				Anims['Run']:Stop()
